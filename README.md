...
5. Output: a `resume.md` per role, ready to submit as a job application.

### Outputs

- Working primary-loop tree with mutation history (`autowrite-<resume-slug>/`)
- Profile cache (`profiles/`)
- Applications tree (`applications/<company>/<role>/`) containing both `resume.md` (markdown source) and `resume.html` (print-ready -- open in browser, Ctrl+P -> Save as PDF) per opening
- Company-level navigation pages (`applications/<company>/index.html`) listing every discovered opening with active-status tags and direct links to the print-ready resumes
...
- changelog.md (primary loop) and sub-changelog.md (per role) -- causal logs of every mutation for interview prep
...
The plugin ships two skills. `autowrite` is the autonomous evaluator + mutator. `career-intake` is a human-in-the-loop interview skill that builds the supplementary-context library `autowrite` reads from. Recommended flow for a fresh resume update: run `/career-intake` first to itemize the candidate's work, then `/autowrite` against the resulting draft.
...
1. **`<chosen-name>.md`** -- the revised resume. Diff against your original, decide what to keep, apply manually to your canonical resume. autowrite never overwrites your original.
2. **`changelog.md`** -- the research log. Each kept mutation has a reason. When an interviewer asks "why is your resume structured this way?" you have receipts.
3. **`recruiter-reports/<final>/<profile>.md`** -- per-profile remaining gaps. These are your **interview prep talking points**. You know in advance what each target company will probe, because their recruiter subagent flagged it.
4. **`<resume-parent-dir>/profiles/<slug>.md`** -- the cached company profile. Reusable on every future run against this resume. Treat it as a living document.
...
- **Single-document scope.** autowrite scores one resume against profiles. It doesn't yet handle cover letters, LinkedIn summaries, or portfolio sites as targets -- though the architecture would extend cleanly.
